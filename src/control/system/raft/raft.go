...
// submitSystemPropsUpdate submits the given system properties update
// the raft service.
func (db *Database) submitSystemPropsUpdate(props map[string]string) error {
...
func (d *dbData) applySystemUpdate(op raftOp, data []byte, panicFn func(error)) {
	...
	case raftOpUpdateSystemProps:
		for k, v := range props {
			if v == "" {
				delete(d.System.Properties, k)
				continue
			}
			d.System.Properties[k] = v
		}
	default:
