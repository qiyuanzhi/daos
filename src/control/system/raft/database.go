...
// SetSystemProps submits an update to the system properties map.
func (db *Database) SetSystemProps(props map[string]string) error {
	if err := db.CheckLeader(); err != nil {
		return err
	}
	db.Lock()
	...
	if err := db.submitSystemPropsUpdate(props); err != nil {
		return err
	}
...
// GetSystemProps returns a copy of the system properties map.
func (db *Database) GetSystemProps(keys []string) (map[string]string, error) {
	if err := db.CheckReplica(); err != nil {
		return nil, err
	}
	db.data.RLock()
	...
	if len(keys) == 0 {
		for k, v := range db.data.System.Properties {
			out[k] = v
		}
		return out, nil
	}
	...
	for _, k := range keys {
		if v, found := db.data.System.Properties[k]; found {
			out[k] = v
			continue
		}
		return nil, system.ErrSystemPropNotFound(k)
	}
	return out, nil
}
